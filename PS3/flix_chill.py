# -*- coding: utf-8 -*-
"""
Created on Sat Mar  5 13:26:59 2022
...
class Movie:
    ...
    def __init__(self, id, date, rank):
        ...
        self.max_rank_info=(rank, id, date)
...
class FlixChillManager(AVL):
    ...
    def maintain(self):
        ...
        if self.left!=None and self.right!=None:
            if self.left.item.max_rank_info[0]<self.right.item.max_rank_info[0]:
                self.item.max_rank_info=self.right.item.max_rank_info
            else:
                self.item.max_rank_info=self.left.item.max_rank_info

    
        elif self.left!=None and self.item.max_rank_info[0]<self.left.item.max_rank_info[0]:
            self.item.max_rank_info=self.left.item.max_rank_info
            
        elif self.right!=None and self.item.max_rank_info[0]<self.right.item.max_rank_info[0]:
            self.item.max_rank_info=self.right.item.max_rank_info
    ...
    def highest(self, s):
        ...
        if self.is_empty(): 
            return None
        if self.subtree_find(s) != None:
            node=self.subtree_find(s)
        else:
            node = self.subtree_close(s) # guarenteed to have item
            if node.item.key < s:
                node = node.tree_successor()
        self.maintain()
        ###FIND MAX RANKING OF CHILDREN
        max_rank_info=(node.item.rank, node.item.id, node.item.key)
        if node.right!=None and node.right.item.max_rank_info[0]>max_rank_info[0] and node.right.item.max_rank_info[2]>=s:
            max_rank_info=node.right.item.max_rank_info

        ###FIND MAX RANK OF ANCESTORS
        while node.parent!=None:

            if node.parent.item.max_rank_info[0]>node.parent.right.item.max_rank_info[0] and node.parent.item.max_rank_info[2]>=s:
                max_rank_info=node.parent.item.max_rank_info
            elif node.parent.item.max_rank_info[0]<node.parent.right.item.max_rank_info[0] and node.parent.right.item.max_rank_info[2]>=s:
                max_rank_info=node.parent.right.item.max_rank_info
            node=node.parent
        while node.right!=None:
            max_right_info=node.item.max_rank_info
            node=node.right
            # if node.right.right.right.right.right.right!=None and node.right.right.right.right.right.right.item.max_rank_info[0]>max_rank_info[0]:
            #     max_rank_info=node.right.right.right.right.right.right.item.max_rank_info
        if max_right_info[0]>max_rank_info[0] and max_right_info[2]>=s:
            max_rank_info=max_right_info
        
        return max_rank_info[1]
